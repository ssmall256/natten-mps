...
class NeighborhoodAttention3DFunction(torch.autograd.Function):
    """Fused 3D neighborhood attention with autograd support."""
    ...
    def backward(ctx, grad_output):
        ...
        logits = ops.na3d_qk_forward(q, k, ctx.kernel_size, ctx.dilation, ctx.stride, ctx.is_causal)
        attn_weights = torch.softmax(logits * scale_value, dim=-1)

        av_result = ops.na3d_av_backward(
        ...
        if av_result is not None:
            ...
            qk_result = ops.na3d_qk_backward(
...
class NA3DQKFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, kernel_size, dilation, stride, is_causal, scale):
        ...
        return ops.na3d_qk_forward(q, k, kernel_size, dilation, stride, is_causal, scale)

    @staticmethod
    def backward(ctx, grad_output):
        q, k = ctx.saved_tensors

        result = ops.na3d_qk_backward(
        ...
        with torch.enable_grad():
            q_ = q.detach().requires_grad_(ctx.needs_input_grad[0])
            k_ = k.detach().requires_grad_(ctx.needs_input_grad[1])
            out = _pure.na3d_qk_forward(q_, k_, ctx.kernel_size, ctx.dilation, ctx.stride, ctx.is_causal, ctx.scale)
            grads = torch.autograd.grad(out, (q_, k_), grad_output, allow_unused=True)
        return grads[0], grads[1], None, None, None, None, None


class NA3DAVFunction(torch.autograd.Function):
    ...
    def forward(ctx, attn, v, kernel_size, dilation, stride, is_causal):
        ...
        return ops.na3d_av_forward(attn, v, kernel_size, dilation, stride, is_causal)

    @staticmethod
    def backward(ctx, grad_output):
        attn, v = ctx.saved_tensors

        result = ops.na3d_av_backward(
        ...
        with torch.enable_grad():
            ...
            out = _pure.na3d_av_forward(attn_, v_, ctx.kernel_size, ctx.dilation, ctx.stride, ctx.is_causal)
