...
class NA1DQKFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, kernel_size, dilation):
        ...
        return ops.na1d_qk_forward(q, k, kernel_size, dilation)
    ...
    def backward(ctx, grad_output):
        ...
        with torch.enable_grad():
            q_ = q.detach().requires_grad_(ctx.needs_input_grad[0])
            k_ = k.detach().requires_grad_(ctx.needs_input_grad[1])
            out = ops.na1d_qk_forward(q_, k_, ctx.kernel_size, ctx.dilation)
...
class NA1DAVFunction(torch.autograd.Function):
    ...
    def forward(ctx, attn, v, kernel_size, dilation):
        ...
        return ops.na1d_av_forward(attn, v, kernel_size, dilation)
    ...
    def backward(ctx, grad_output):
        ...
        with torch.enable_grad():
            ...
            out = ops.na1d_av_forward(attn_, v_, ctx.kernel_size, ctx.dilation)
