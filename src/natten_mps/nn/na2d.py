...
class NeighborhoodAttention2D(torch.nn.Module):
    ...
    def forward(self, x, spatial_sizes=None):
        ...
        Args:
            ...
            spatial_sizes: Optional ``[B, 2]`` int tensor of actual (H, W) per
                batch element for variable-length attention.  Positions beyond
                the per-sample sizes produce zero output.
        """
        if x.ndim != 4:
        ...
        if self._use_gqa:
            q = self.q_proj(x).reshape(bsz, height, width, self.num_heads, self.head_dim)
            kv = self.kv_proj(x).reshape(bsz, height, width, 2, self.num_kv_heads, self.head_dim)
            kv = kv.permute(3, 0, 1, 2, 4, 5)
            k, v = kv.unbind(0)
        else:
        ...
        out = out.reshape(bsz, out.shape[1], out.shape[2], self.embed_dim)
        out = self.proj(out)
